...
// Event listeners to close the alert modal
...
document.getElementById("alertModal")?.addEventListener("click", (e) => {
  if (e.target.id === "alertModal") hideAlert(); // click backdrop to close
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!input) {
    showAlert("Please type an address first");
    return;
  }
  ...
  geocoder.geocode({ address: input }, (results, status) => {
    if (status === "OK" && results[0]) {
      ...
      document.getElementById("saveNoteBtn").onclick = function () {
        ...
        if (!userNote) {
          showAlert("You have not added your note");
          return;
        }
        ...
        try {
          ...
          showAlert("Could not save marker. Check Firebase setup.");
        }
      };
    } else {
      showAlert("Address not found: " + status);
    }
  });
});

  // Cerrar la ventana emergente al hacer clic en la "X" del modal principal
  ...
  if (lastAddedKey) {
    ...
    remove(ref(db, `markers/${lastAddedKey}`))
      ...
      .catch((error) => {
        ...
        showAlert("Error deleting the mark");
      });
  } else {
    showAlert("There is no recent last mark recorded.");
  }
});
;
